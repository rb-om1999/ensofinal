...
async def capture_chart_screenshot(url: str) -> dict:
    """Capture chart screenshot using Playwright + Browserless.io WebSocket connection"""
    try:
        browserless_token = "2TNtEb9P4h0cSGo56221aad712d3ad88f38b622ee09c4e707"
        browserless_endpoint = f"wss://production-sfo.browserless.io/chromium/playwright?token={browserless_token}"
        
        logger.info(f"Connecting to browserless.io for URL: {url}")
        
        async with async_playwright() as playwright:
            # Connect to browserless.io via WebSocket
            browser = await playwright.chromium.connect(browserless_endpoint)
            
            try:
                # Create new context with mobile viewport initially, then desktop
                ...
                # Try to handle TradingView-specific elements
                if 'tradingview.com' in url.lower():
                    try:
                        # Wait for chart canvas to load
                        await page.wait_for_selector('canvas', timeout=10000)
                        logger.info("Chart canvas detected")
                        ...
                        # Wait a bit more for chart to stabilize after closing popups
                        await page.wait_for_timeout(2000)
                        
                    except Exception as tv_error:
                        logger.warning(f"TradingView specific handling failed: {tv_error}")
                ...
                logger.info(f"Screenshot captured successfully, size: {len(screenshot_buffer)} bytes")
                
                # Convert to base64
                base64_string = base64.b64encode(screenshot_buffer).decode('utf-8')
                ...
                return {
                    ...
                    "width": width,
                    "height": height,
                    "size_bytes": len(screenshot_buffer),
